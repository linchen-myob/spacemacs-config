...
(use-package mocha
  ...
  ;; (defcustom mocha-jest-command "jest --config test/jest.config.functional.js --env=jsdom --colors"
  (defcustom mocha-jest-command "jest --config configs/jest.json --env=jsdom --colors"
...
;; only need override it in ts mode
(defun my-jest-find-current-test()
...
(advice-add 'mocha-find-current-test :override 'my-jest-find-current-test)
